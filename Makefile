...
# TARGETS
# =========================
.PHONY: all clean server test run-server run-test emcc napi compile_lib
...
run-server: $(SERVER_BIN_DIR)/server
	@./$(SERVER_BIN_DIR)/server $(PORT)

$(SERVER_BIN_DIR)/%.o: $(SERVER_SRC_DIR)/%.cpp
	@mkdir -p $(dir $@)
	$(CXX) $(CXXFLAGS_NATIVE) $(UWS_INC) -I. \
		-DCOMPILE_SERVER=1 -DTESTING_SERV=$(TEST_SERV) -DRELEASE_SERV=$(RELEASE_SERV) -DMAP_PRECISION=$(SERV_MAP_PRECISION) \
		-c $< -o $@
...
$(GAME_BIN_DIR)/napi/%.o: $(GAME_SRC_DIR)/%.cpp
	@mkdir -p $(dir $@)
	$(CXX) $(CXXFLAGS) -fPIC -I$(NODE_ADDON_API_DIR) -I$(NODE_INCLUDE_DIR) \
		-DPRODUCE_NAPI=1 -c $< -o $@
...
compile_lib:
	@mkdir -p $(GAME_BIN_DIR)
	$(CXX) -shared -fPIC $(GAME_SRC) $(CXXFLAGS_NATIVE) \
		-o $(GAME_BIN_DIR)/api.$(LIB_EXT) $(LDFLAGS_NATIVE)

# =========================
# SYNC / GRAPH
# =========================
.PHONY: watchDef graphActivities
